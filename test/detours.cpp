...
bool RegisterDetours(void* DetourData, int NumDetours, INT64 scriptOffset)
{
...
#ifdef DETOUR_LOGGING
	std::cout << "Registering %d detours in script %p...", NumDetours, scriptOffset;
#endif

	INT64 base = (INT64)DetourData;
	std::cout << "detour data: " << DetourData;
	for (int i = 0; i < NumDetours; i++)
	{
		...
		printf("Detour Parsed: FixupName:%x, ReplaceNamespace:%x, ReplaceFunction:%x, FixupOffset:%x, FixupSize:%x} {FixupMin:%p, FixupMax:%p", read_detour->FixupName, read_detour->ReplaceNamespace, read_detour->ReplaceFunction, read_detour->FixupOffset, read_detour->FixupSize, detour->hFixup, detour->hFixup + detour->FixupSize);
