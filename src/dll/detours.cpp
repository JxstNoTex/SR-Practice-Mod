...
bool RegisterDetours(void* DetourData, int NumDetours, long long scriptOffset)
{
	GSCBuiltins::nlog("scriptOffset: %x", scriptOffset);
	RemoveDetours();
	ScriptDetours::GSC_OBJ = (char*)scriptOffset;

	std::cout << "Registering " << NumDetours << " detours in script " << scriptOffset << "...";
	std::cout << std::endl;
